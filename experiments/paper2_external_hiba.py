...
"""
import json, math, re, tarfile, hashlib, urllib.request, zipfile
...
from sklearn.metrics import accuracy_score, f1_score, recall_score, roc_auc_score, average_precision_score, confusion_matrix
...
# Pre-specified taxonomy mapping. SCC is intentionally unsupported/excluded.
DX_MAP={
 'actinic keratosis':'akiec','basal cell carcinoma':'bcc','seborrheic keratosis':'bkl',
 'solar lentigo':'bkl','lichenoid keratosis':'bkl','dermatofibroma':'df','melanoma':'mel',
 'nevus':'nv','vascular lesion':'vasc'
}
...
def fit_temperature(p,y):
 ...
 res=minimize_scalar(lambda t:nll(apply(float(t)),y),bounds=(.1,8),method='bounded',options={'xatol':1e-5});return float(res.x)
...
def seed_dir(seed):
 if seed==2026:return BASE/'grouped_mc_seed2026'
 root=BASE/'multiseed_fixed_split'
 for d in [root]+[x for x in root.rglob('*') if x.is_dir()]:
  if re.search(rf'(?<!\d){seed}(?!\d)',str(d)) and (d/'best.pt').exists():return d
 raise FileNotFoundError(seed)
def load_internal(seed):
 d=seed_dir(seed);z=np.load(d/'mc_predictions.npz',allow_pickle=False);return z
class Net(nn.Module):
 def __init__(self):
...
def model_for(seed,device):
 m=Net().to(device);obj=torch.load(seed_dir(seed)/'best.pt',map_location=device,weights_only=False);st=obj.get('model_state',obj.get('state_dict',obj)) if isinstance(obj,dict) else obj;st={k.replace('module.','',1) if k.startswith('module.') else k:v for k,v in st.items()};m.load_state_dict(st,strict=True);m.eval();return m
...
def cluster_boot(p,y,score,les,thr,B=1000,seed=20260811):
 ...
 point=metrics(p,y,score);point.update({'sel_'+k:v for k,v in safety(p,y,score,thr).items()});rows=[]
 for k,v in vals.items():rows.append({'metric':k,'point':point.get(k,np.nan),'ci_low':float(np.quantile(v,.025)),'ci_high':float(np.quantile(v,.975))})
 return rows
...
def reliability(p,y):
 ...
 for kind,prob,target in [('overall',cf,ok),('melanoma_vs_rest',p[:,MEL],y==MEL)]:
  for i in range(10):
   m=(prob>=ed[i])&((prob<ed[i+1]) if i<9 else (prob<=ed[i+1]))
   rows.append({'kind':kind,'bin_low':ed[i],'bin_high':ed[i+1],'n':int(m.sum()),'mean_probability':float(prob[m].mean()) if m.any() else np.nan,'observed_frequency':float(target[m].mean()) if m.any() else np.nan})
 return rows
def aggregate_lesion(frame,p):
 tmp=frame[['lesion_id','ham_class']].copy();tmp['y']=[IDX[x] for x in tmp.ham_class];out=[];yy=[]
 for lid,g in tmp.groupby('lesion_id'):
  ix=g.index.to_numpy();labs=np.unique(g.y)
  if len(labs)!=1:continue
  out.append(p[ix].mean(0));yy.append(labs[0])
 return norm(np.asarray(out)),np.asarray(yy,int)
def main():
 ...
 if not BUNDLE.exists():
  print('HIBA_DOWNLOAD_START',flush=True);urllib.request.urlretrieve(ZIP_URL,BUNDLE);print('HIBA_DOWNLOAD_DONE|'+str(BUNDLE.stat().st_size),flush=True)
 ...
 df=pd.read_csv(META);df['diagnosis_norm']=df.diagnosis.astype(str).str.strip().str.lower();df['ham_class']=df.diagnosis_norm.map(DX_MAP)
 mapping=df.groupby(['diagnosis_norm','ham_class'],dropna=False).size().reset_index(name='n');mapping.to_csv(OUT/'hiba_taxonomy_mapping_all_images.csv',index=False)
 # Primary external cohort: dermoscopic modality + compatible seven-class diagnoses.
 use=df[(df.image_type.astype(str).str.lower()=='dermoscopic') & df.ham_class.notna()].copy().reset_index(drop=True)
 excluded=df[~((df.image_type.astype(str).str.lower()=='dermoscopic') & df.ham_class.notna())][['isic_id','image_type','diagnosis','lesion_id','patient_id']].copy();excluded.to_csv(OUT/'hiba_exclusions.csv',index=False)
 ...
 if miss:raise FileNotFoundError(f'{len(miss)} mapped HIBA images missing, e.g. {miss[:5]}')
 print('HIBA_PRIMARY|'+json.dumps({'images':len(use),'lesions':use.lesion_id.nunique(),'patients':use.patient_id.nunique(),'class_counts':use.ham_class.value_counts().to_dict(),'excluded':len(excluded)}),flush=True)
 ...
 members=[]
 for s in SEEDS:
  m=model_for(s,device);p,y,ids=predict_det(m,ld,device);members.append(p);print('HIBA_MEMBER_DONE|'+str(s),flush=True);del m;torch.cuda.empty_cache() if torch.cuda.is_available() else None
 members=np.stack(members,1);ens=norm(members.mean(1));y=np.asarray(y);les=use.lesion_id.astype(str).to_numpy()
 # Internal validation scores/thresholds are frozen before HIBA evaluation.
 final=np.load(BASE/'final_stage'/'deep_ensemble_predictions.npz',allow_pickle=False);iv=norm(final['val_member_probs'].mean(1));ivy=load_internal(11)['val_labels'].astype(int);ens_thr=threshold_for_coverage(1-iv.max(1),.60);temp=fit_temperature(iv,ivy);ivc=apply_temperature(iv,temp);ens_cal_thr=threshold_for_coverage(1-ivc.max(1),.60);ens_safety10=constrained_threshold(iv,ivy,1-iv.max(1),.10)
 # Seed-2026 MC predictive entropy; threshold from HAM validation only.
 m=model_for(2026,device);mc_ext,ymc,idmc=predict_mc(m,ld,device,30);del m;torch.cuda.empty_cache() if torch.cuda.is_available() else None
 mp_ext,ms_ext=mc_scores(mc_ext);z26=load_internal(2026);ivmc=z26['val_mc_probs'];ivmp,ivms=mc_scores(ivmc);mc_thr=threshold_for_coverage(ivms['pe'],.60);mc_safety10=constrained_threshold(ivmp,z26['val_labels'].astype(int),ivms['pe'],.10)
 methods={
  'deep_ensemble_msp':(ens,1-ens.max(1),ens_thr),
  'deep_ensemble_calibrated_msp':(apply_temperature(ens,temp),1-apply_temperature(ens,temp).max(1),ens_cal_thr),
  'mc_dropout_pe_seed2026':(mp_ext,ms_ext['pe'],mc_thr)
 }
 summary={'source':{'name':'HIBA Skin Lesions','doi':'10.34970/559884','official_images':1635,'primary_modality':'dermoscopic'},'mapping':mapping.fillna('EXCLUDED').to_dict('records'),'primary_cohort':{'images':int(len(use)),'lesions':int(use.lesion_id.nunique()),'patients':int(use.patient_id.nunique()),'class_counts':{k:int(v) for k,v in use.ham_class.value_counts().items()},'excluded_images':int(len(excluded))},'frozen_internal':{'ensemble_threshold_60':ens_thr,'ensemble_temperature':temp,'ensemble_calibrated_threshold_60':ens_cal_thr,'ensemble_safety10':ens_safety10,'mc_pe_threshold_60':mc_thr,'mc_safety10':mc_safety10},'methods':{}}
 cirows=[];pcrows=[];relrows=[]
 for j,(name,(p,score,thr)) in enumerate(methods.items()):
  d=metrics(p,y,score);d['frozen_60']=safety(p,y,score,thr);summary['methods'][name]=d
  x=cluster_boot(p,y,score,les,thr,1000,20260811+j);[r.update({'method':name}) for r in x];cirows+=x
  x=per_class(p,y,score,thr);[r.update({'method':name}) for r in x];pcrows+=x
  x=reliability(p,y);[r.update({'method':name}) for r in x];relrows+=x
  risk_curve(p,y,score).to_csv(OUT/f'risk_coverage_{name}.csv',index=False)
  lp,ly=aggregate_lesion(use,p);ls=1-lp.max(1);summary['methods'][name]['lesion_level']={'n':int(len(ly)),'accuracy':float(accuracy_score(ly,lp.argmax(1))),'macro_f1':float(f1_score(ly,lp.argmax(1),average='macro',zero_division=0)),'melanoma_recall':float(recall_score(ly==MEL,lp.argmax(1)==MEL,zero_division=0)),'aurc_msp':aurc(ls,lp.argmax(1),ly),'eaurc_msp':eaurc(ls,lp.argmax(1),ly)}
 # Safety-constrained thresholds (10% validation automatic melanoma miss) evaluated frozen externally.
 ...
 # External subgroup robustness for ensemble (descriptive only).
 sg=[];epr=ens.argmax(1)
 for col in ['sex','anatom_site_general','diagnosis_confirm_type','fitzpatrick_skin_type']:
  for val,g in use.groupby(col,dropna=False):
   ix=g.index.to_numpy()
   if len(ix)>=20:sg.append({'subgroup_variable':col,'subgroup':str(val),'n':len(ix),'accuracy':accuracy_score(y[ix],epr[ix]),'macro_f1':f1_score(y[ix],epr[ix],average='macro',zero_division=0),'melanoma_n':int((y[ix]==MEL).sum()),'melanoma_recall':recall_score(y[ix]==MEL,epr[ix]==MEL,zero_division=0) if (y[ix]==MEL).any() else np.nan,'ece15':ece(ens[ix],y[ix])})
 pd.DataFrame(cirows).to_csv(OUT/'hiba_lesion_cluster_ci.csv',index=False);pd.DataFrame(pcrows).to_csv(OUT/'hiba_class_specific_referral.csv',index=False);pd.DataFrame(relrows).to_csv(OUT/'hiba_reliability_bins.csv',index=False);pd.DataFrame(sg).to_csv(OUT/'hiba_subgroup_robustness.csv',index=False)
 use[['isic_id','lesion_id','patient_id','image_type','diagnosis','ham_class','sex','age_approx','anatom_site_general','diagnosis_confirm_type','fitzpatrick_skin_type']].to_csv(OUT/'hiba_primary_cohort.csv',index=False)
 np.savez_compressed(OUT/'hiba_predictions.npz',image_ids=use.isic_id.astype(str).to_numpy(),labels=y,ensemble_member_probs=members.astype(np.float32),ensemble_probs=ens.astype(np.float32),mc_seed2026_probs=mc_ext.astype(np.float32))
 (OUT/'hiba_external_summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True),encoding='utf-8')
 ...
 print('HIBA_EXTERNAL_SUMMARY|'+json.dumps({'archive':str(ARCHIVE),'size':ARCHIVE.stat().st_size,'sha256':h,'primary':summary['primary_cohort'],'ensemble':summary['methods']['deep_ensemble_msp'],'mc':summary['methods']['mc_dropout_pe_seed2026']}),flush=True);print('PAPER2_HIBA_EXTERNAL_DONE',flush=True)
