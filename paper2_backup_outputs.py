...
def main():
    ...
    index={'manifest_path':str(manifest_path),'manifest_sha256':sha256_file(manifest_path),'archives':archives,'source_file_count':manifest['file_count'],'source_total_bytes':manifest['total_bytes'],'seed_status':seed_status()}
    index_path=EXPORT/'paper2_backup_index.json'; index_path.write_text(json.dumps(index,indent=2,sort_keys=True),encoding='utf-8')
    # verify every archive against the recorded digest
